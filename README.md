...
NETRA v2 is optimized to run the **full pipeline in ~5 seconds** on Jetson Orin Nano 8GB, meeting the VYOMA Competition requirement of **< 6 second** end-to-end latency.
...
| **LLM Inference** | **Qwen2.5-1.5B (Q4_K_M)** | **~2-3s** |
...
| # | Optimization | Impact |
|---|---|---|
| 1 | **Fast LLM: Qwen2.5-1.5B** instead of Qwen3-4B | ~2-3x faster inference, ~1 GB smaller |
| 2 | **Short answers: 150 tokens max** (2-3 sentences) | 150 tokens at ~100 tok/s = 1.5s |
| 3 | **NMT eliminated** — LLM answers in the user's language directly | Saves an entire LLM call (~3-4s) |
| 4 | **Minimal RAG context** — top_k=1, max 500 chars | Less prompt tokens = faster prompt eval |
| 5 | **No query translation** — embedding model handles multilingual natively | Saves another LLM call (~3-4s) |
| 6 | **Compact prompts** — 80% shorter system/user prompts | Fewer input tokens = faster |
...
defaults:
  llm: "ollama_qwen2_5_1_5b"    # Fast 1.5B model for edge

inference:
  max_output_length: 150          # Short, focused answers
  timeout_seconds: 10             # Fail fast
```

> **Note:** On first request after server start, models are loaded into GPU memory (~8-10s). All subsequent requests run at full speed (~3-5s). The server pre-warms models automatically on startup.
...
ollama pull qwen2.5:1.5b            # Fast LLM for Jetson (recommended)
...
| 4 | **LLM (Jetson)** | **Qwen2.5-1.5B** | **Q4_K_M (INT4)** | GGUF / Ollama | **1.0 GB** | `ollama pull qwen2.5:1.5b` |
...
ollama pull qwen2.5:1.5b            # Fast LLM for Jetson  (Q4_K_M, 1.0 GB)
ollama pull qwen3:4b                # Full LLM             (Q4_K_M, 2.6 GB)
...
|---|---|---|---|
| **Qwen2.5-1.5B** | `ollama pull qwen2.5:1.5b` | ~1.0 GB | **Default LLM (Jetson — fast edge inference)** |
...
| LLM | 22 | **`ollama_qwen2_5_1_5b`** | `ollama_qwen3_4b` | `ollama_qwen2_5_3b` |
...
|---|---|---|---|---|---|
| `jetson_orin_nano` | 6 GB | PaddleOCR | **Qwen2.5-1.5B** | **~5s** | Optimized for speed, NMT skipped |
